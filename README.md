...
《星之旅Lunarium》的非官方全收集交互地图。支持读取本地存档后解析尚未收集的物品。
...
- 在游戏原生地图上显示未收集的道具位置。
- 标记会跟随地图切换、拖动和缩放，数字对应全收集指南编号。
- 鼠标悬停标记可查看道具名称和未收集判定依据。
- 左上角列出当前地图的全部未收集道具；切换地图后会自动更新。
- 结合地图拾取、商店库存、任务、剧情、Boss 进度等存档信号逐点判断。
- 只读取游戏内存中的存档数据，不会修改或写入存档。
...
运行时流程为：读取 `SaveManager.CurrentData` → 分析逐点完成状态 → 匹配已经加载的 `LevelRegion` → 创建或移除原生地图 UI 标记 → 更新当前地图的遗漏列表。整个流程只读，不调用游戏的保存接口。
